...
def test_validate_node_accepts_person() -> None:
    node = validate_node(NodeSpec("Person", "person:chandra", {"name": "Chandra"}))
    assert node.key == "person:chandra"
...
def test_validate_edge_checks_endpoints() -> None:
    edge = EdgeSpec("WORKS_ON", "person:chandra", "venture:trading-firm")
    validate_edge(edge, known_keys={"person:chandra", "venture:trading-firm"})
