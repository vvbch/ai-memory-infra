...
tested locally; droplet deploy + live verification in progress this session**). Repo-health
...
**This session built it (ADR 035):** the endpoint is now its own OAuth 2.1 authorization
server — discovery, dynamic client registration, PKCE — using the auth framework already
inside our pinned MCP SDK (no new vendor, no new dependency). The only new code is the
single-user part: a consent page where you paste the existing connector secret to approve
Claude, and a small state file (hashed tokens, Docker volume) so the connector survives
redeploys. Your old token still works for curl tests. 25 new tests; suite 103 passed,
94% coverage; lint/types clean.
...
**Goal 2 of the 2026-06-10 day plan — remote MCP for Claude iPhone.** OAuth build is
**done and tested**; next is droplet deploy + live verify, then the two operator steps:
Bitwarden save of `MCP_CONNECTOR_BEARER_TOKEN`, claude.ai connector registration (now an
...
- **Infra:** compose volume + `MCP_PUBLIC_BASE_URL`; Caddyfile/Dockerfile/.env.example
  ...
  private interview-packet decision log).
...
- **Goal 2 close-out:** droplet deploy + live verify (this session), then operator steps —
  Bitwarden save of the secret, claude.ai OAuth connect (consent page), iPhone round-trip.
...
> **RESUME HERE — finish Goal 2:** (1) deploy the OAuth build to the droplet (git pull →
> `docker compose -f docker-compose.yml -f docker-compose.prod.yml build mcp-proxy` →
> `up -d mcp-proxy`) and live-verify: OAuth discovery 200 on
> `https://mcp.chandrav.dev/.well-known/oauth-authorization-server`, MCP route 401 with
> `resource_metadata` challenge, static-token `search_memories` still works. (2) Then the
> operator steps, one at a time (concierge): Bitwarden save of
> `MCP_CONNECTOR_BEARER_TOKEN`; claude.ai → Add custom connector
> (URL `https://mcp.chandrav.dev/`, OAuth fields empty) → approve on the consent page;
> confirm on iPhone with one live `search_memories` round-trip.
>
