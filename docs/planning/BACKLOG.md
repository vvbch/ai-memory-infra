...
- ✅ **`[backup]` Phase 2 backup/restore + drills — DONE.** Automating backups +
  restore drill was re-scoped *into* Phase 2 (ADR 023). ✅ §1 daily timer + ✅ §2
  dead-man's-switch (healthchecks.io) + ✅ §3 data-loss hardening (versioning +
  lifecycle, least-privilege backup key, pre-restore snapshot) + ✅ §4 monthly
  restore drill (systemd timer + failure watchdog). Closed in BUILD-JOURNEY /
  AGENTS.md Phase 2. Tie: tenets 4/17, ADR 022 + ADR 023.
