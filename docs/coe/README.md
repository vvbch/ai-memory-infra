...
| 2026-06-10 | `2026-06-10-session-end-commit-permission-ask.md` | After ChatGPT went live, agent asked "say if you want that committed" despite standing session-end commit+push authorization; droplet was ahead of git (SCP deploy) | AGENTS.md sharpened: never permission-ask at handoff; commit+push executed; deploy-via-git follow-up in BACKLOG |
| 2026-06-10 | `2026-06-10-mcp-timeout-semantic-duplicates.md` | MCP bulk-write client timeout treated as failure → reworded retries created 3 semantic dupes; metrics batch partially landed; MCP lacked delete/update | ADR 037 + tenet 19 + bulk_seed_importer + memory_compaction (review-first) + MCP/CLI delete/update |
| 2026-06-10 | `2026-06-10-global-workspace-rule-conflict.md` | Global Cursor user rule ("only commit when asked") fought AGENTS.md standing authorization across 4+ handoff COEs; workspace fixes never updated the global rule | AGENTS.md park + conflict protocol; operator replaces global rule text per COE § Global rule replacement |
| 2026-06-10 | `2026-06-10-hallucinated-cursor-rule-name.md` | Agent cited `committing-changes-with-git` as a Cursor Settings rule name — it is an internal prompt tag, not a UI label | COE corrected; never cite prompt tags as Settings paths |
| 2026-06-10 | `2026-06-10-clipboard-over-print-delegation.md` | Agent claimed clipboard handoff worked; operator saw empty clipboard | `cursor_user_rules.py` print-first; concierge defaults to text in chat |
| 2026-06-10 | `2026-06-10-cursor-user-rules-not-on-disk.md` | Agent implied global user rules are local files; grep/state.vscdb found no commit-rule strings; rules are cloud KnowledgeBase | Document layers; workspace AGENTS.md + hooks are the enforceable fix |
