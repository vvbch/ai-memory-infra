...
| GitHub — repo + Actions (CI) | Source of truth, CI on PRs, weekly eval job; **CD is manual SSH today** | **(₹0)** (free for public repo) |
...
|---|---|---|
| `memory.{domain}` | Mem0 REST API | JWT + admin API key; CORS allowlist **[target]** |
